...
my_api_key = "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"  # 在这里输入你的 API 密钥
...
logging.info(
    colorama.Back.GREEN
    + "\n川虎的温馨提示：访问 http://localhost:8080 查看界面"
    ...
    if dockerflag:
        if authflag:
            demo.queue(concurrency_count=CONCURRENT_COUNT,api_open=False).launch(
                server_name="0.0.0.0", server_port=8080, auth=(username, password),
                favicon_path="./assets/favicon.png"
            )
        else:
            demo.queue(concurrency_count=CONCURRENT_COUNT,api_open=False).launch(server_name="0.0.0.0", server_port=8080, share=False, favicon_path="./assets/favicon.png")
    # if not running in Docker
    else:
        if authflag:
            demo.queue(concurrency_count=CONCURRENT_COUNT,api_open=False).launch(share=False, server_port=8080, auth=(username, password), favicon_path="./assets/favicon.png", inbrowser=True)
        else:
            demo.queue(concurrency_count=CONCURRENT_COUNT,api_open=False).launch(share=False, server_port=8080, favicon_path="./assets/favicon.ico", inbrowser=True)  # 改为 share=True 可以创建公开分享链接
